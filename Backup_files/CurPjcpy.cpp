...
void FindContour (Dots* dots, int fx , int fy)
{
  ...
  // left direction
  if ( ( current % fx != 0) && (dots[current % fx].exists == true) )
    dfs(current - 1, st);

  // right direction
  if (((current + 1) % fx != 0) && (dots[(current + 1) % fx].exists == true))               
    dfs(current + 1, st);
  ...
  // down direction
  if (( current + fx < fx * fy ) && (dots[current + fx].exists == true))
    dfs(current + fx, st);

  //up-left direction 
  if (( current % fx != 0) && ( current - fx >= 0)
     && (dots[current - 1 - fx].exists == true))
    dfs(current - 1 - fx, st);
